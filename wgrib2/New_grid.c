...
/*
 * HEADER:111:new_grid_winds:misc:1:new_grid wind orientation: X = grid, earth (N/S, default) -- alpha
 */
...
static const char *default_vectors[] = {"UGRD", "VGRD", "VUCSH", "VVCSH","UFLX", "VFLX",
...
static char **vectors = (char **) default_vectors;

/*
 * HEADER:111:new_grid_vectors:misc:1:change fields to vector interpolate: X=none,default,UGRD:VGRD
 */

int f_new_grid_vectors(ARG1) {
    if (mode < -1) return 0;
    if (strcmp(arg1,"none") == 0)  vectors = (char **) no_vectors;
    else if (strcmp(arg1,"default") == 0)  vectors = (char **) default_vectors;
    else if (strcmp(arg1,"UGRD:VGRD") == 0)  vectors = (char **) UV_vectors;
    else fatal_error("new_grid_vectors: %s is not understood", arg1);
    return 0;
}
...
// fprintf(stderr," UV interpolation %s , %s\n", save->name, name);
	}
	else {
	    for (i = 0; i < npnts; i++) {
